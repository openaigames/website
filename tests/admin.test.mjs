...
test('GitHub sessions, immutable admin identity, moderation privacy, CSRF and concurrent review',async()=>{
 ...
 codeUser='member-token';const memberLogin=await call('/api/auth/github/login?return=https://evil.example');const memberState=new URL(memberLogin.headers.get('Location')).searchParams.get('state');const memberCallback=await call('/api/auth/github/callback?code=code&state='+memberState,{headers:{Cookie:'__Host-oag_admin_oauth='+memberState}});assert.equal(memberCallback.headers.get('Location'),'/');assert.match(memberCallback.headers.get('Set-Cookie'),/__Host-oag_admin_session=/);
 const commentLogin=await call('/api/auth/github/login?'+new URLSearchParams({return:'/?lang=en#/comments/dodo'}));const commentState=new URL(commentLogin.headers.get('Location')).searchParams.get('state');const commentCallback=await call('/api/auth/github/callback?code=code&state='+commentState,{headers:{Cookie:'__Host-oag_admin_oauth='+commentState}});assert.equal(commentCallback.headers.get('Location'),'/?lang=en#/comments/dodo');
 const memberCount=await db.prepare('SELECT COUNT(*) as count FROM admin_sessions WHERE github_id=888').first();assert.equal(memberCount.count,3);
