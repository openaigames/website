...
(() => {
  ...
  const tools = [{
    name: 'list_playable_games', title: '查找可玩的游戏',
    ...
    async execute(input) {
      ...
      const { projects } = await window.OpenAIGamesCloud.catalog();
      return { games: projects.filter(p => `${p.title} ${p.description} ${p.category}`.toLowerCase().includes(query.toLowerCase())).map(p => ({ id: p.id, title: p.title, category: p.category, description: p.description })) };
    }
  }, {
    name: 'open_game_for_play', title: '插卡并打开游戏',
    ...
    async execute(input) {
      ...
      return { id: game.id, title: game.title, status: 'ready_to_start' };
    }
  }];
