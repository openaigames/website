(() => {
  ...
  function mountInbox(container, signal, projects = []) {
    ...
    container.innerHTML = `<section class="submission-inbox" aria-label="试玩收件箱"><div class="inbox-heading"><div><p class="site-kicker">FRESH FROM THE COMMUNITY</p><h2>试玩收件箱</h2><p>通过审核的社区试玩。完整作品档案可以继续通过 PR 补齐。</p></div><a class="site-button" href="#/submit">＋ 快捷投稿</a></div><div class="submission-list"></div><p class="inbox-status" role="status"></p><button class="site-button inbox-more" hidden>再看一些</button></section>`;
    ...
    const render = () => {
      list.innerHTML = entries.length ? entries.map(entry => {
        ...
        return `<article class="submission-card"><div class="submission-meta"><span>NO. ${String(entry.id).padStart(3,'0')}</span><span>${published ? '已收录' : '已通过审核'}</span></div><h3>${esc(entry.title)}</h3><p>${esc(entry.description)}</p><div class="submission-byline"><span>${entry.relation === 'creator' ? '作者自荐' : '社区推荐'}</span>${entry.submitter ? ' · <span '+(entry.submitter==='社区整理'?'':'data-i18n-ignore')+'>'+esc(entry.submitter)+'</span>' : ''}</div><button type="button" class="submission-play in-room-play" data-site-play="${esc(published?.id||'inbox-'+entry.id)}">全屏试玩 <span aria-hidden="true">→</span></button><small>${esc(new URL(entry.url).hostname)}</small>${published ? `<a class="site-text-link" href="#/title/${encodeURIComponent(published.id)}">查看卡带档案 →</a>` : ''}</article>`;
  ...
  function mount(container, signal) {
    ...
    form.addEventListener('submit', async event => {
      ...
      try {
        ...
        if (signal.aborted && window.OpenAIGamesSite?.page==='submit') window.OpenAIGamesSite.refresh();
      }
    },{signal});
