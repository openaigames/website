...
(() => {
  ...
  header.innerHTML = `<a class="site-brand" href="#/discover" aria-label="OpenAIGames 首页">OpenAI<span>Games</span><small>一起，做点好玩的。</small></a><nav aria-label="站点导航"><a href="#/discover" data-site-nav="home">精选游戏</a><a href="#/games" data-site-nav="games">游戏目录</a><a href="#/board" data-site-nav="board">许愿 / 留言</a><a href="#/community" data-site-nav="community">如何共创</a><a class="site-github" href="${community}" target="_blank" rel="noopener">GitHub ↗</a></nav>`;
  ...
  let routeId = 0, request, pageKind = '', projects = [], category = '全部', query = '', opening = false;
  ...
  function renderCards() {
    ...
    view.querySelector('#catalog-count').textContent = `${filtered.length} 张可玩的卡带`;
    ...
    list.innerHTML = filtered.length ? filtered.map(project => `<article class="catalog-game"><a class="cartridge-link" href="#/title/${encodeURIComponent(project.id)}" aria-label="查看${esc(project.title)}">${cover(project)}</a><div class="catalog-meta"><span>${esc(project.category)} · ${sourceLabel(project)}</span><a href="#/title/${encodeURIComponent(project.id)}" aria-label="查看${esc(project.title)}详情">↗</a></div><h2><a href="#/title/${encodeURIComponent(project.id)}">${esc(project.title)}</a></h2><p>${esc(project.description)}</p><button class="catalog-play" data-site-play="${esc(project.id)}" aria-label="插卡试玩${esc(project.title)}">插卡试玩 <span aria-hidden="true">→</span></button></article>`).join('') : '<div class="catalog-empty"><h2>这次没找到合适的卡带。</h2><p>换个关键词，或看看所有游戏。</p><button class="site-button" data-site-reset>清空筛选</button></div>';
  }
  async function catalog(signal) {
    ...
    mount(`<section class="site-content catalog-content"><div class="catalog-heading"><div><p class="site-kicker">THE CARTRIDGE COLLECTION</p><h1>挑一张，开玩。</h1><p class="site-lead">从一个小玩法，遇见下一个好点子。</p></div><div class="collection-number" aria-label="${projects.length} 款游戏"><b>${String(projects.length).padStart(2,'0')}</b><span>可玩卡带</span></div></div><div class="catalog-tools"><div class="category-tabs" role="group" aria-label="游戏类型">${categories.map(name => `<button data-site-category="${esc(name)}" aria-pressed="${name===category}">${esc(name)}</button>`).join('')}</div><label class="catalog-search"><span class="sr-only">搜索游戏</span><input id="site-search" type="search" placeholder="找一款游戏…" autocomplete="off" value="${esc(query)}"><span aria-hidden="true">⌕</span></label></div><div class="catalog-caption"><span id="catalog-count" role="status" aria-live="polite"></span><p>来自社区的真实作品，下一张也可以是你的。</p></div><div id="site-game-grid" class="site-game-grid"></div><aside class="catalog-invite"><div><span class="site-kicker">THE NEXT CARTRIDGE</span><h2>这一格，留给你的想法。</h2></div><a class="site-button" href="#/board">写下一个愿望 <span>↗</span></a></aside></section>`);
    renderCards();
  }
  ...
  function participation() {
    mount(`<section class="community-hero"><div class="site-content"><span class="site-kicker">LET’S MAKE SOMETHING PLAYABLE</span><h1>好玩的想法，<br>值得被做出来。</h1><p>先把点子做成能玩的游戏，再一起把它变得更好。<br>这里是 OpenAIGames，一个围绕 AI 与游戏创作的开源社区。</p>${external(community,'走进 OpenAIGames 社区','site-button light')}<span class="community-stamp" aria-hidden="true">INSERT<br>YOUR<br>IDEA.</span></div></section><section class="site-content community-content"><div class="community-section-heading"><span class="site-kicker">YOUR NEXT MOVE</span><h2>从你手上的那一点开始。</h2></div><div class="participation-row"><span class="step-number">01</span><div><h3>有一个点子</h3><p>说说玩家要做什么、哪里好玩。找几个同路人，一起做出第一版。</p></div><a href="#/board" class="site-row-link">写下游戏愿望 ↗</a></div><div class="participation-row"><span class="step-number">02</span><div><h3>已经能玩了</h3><p>带上试玩地址、封面和玩法说明，提一个 PR。自动生成预览，合并后出现在游戏目录。</p></div>${external(community+'/blob/main/docs/GAME_SUBMISSION.md','投稿与预览指南','site-row-link')}</div><div class="participation-row"><span class="step-number">03</span><div><h3>遇到反复出现的问题</h3><p>把创作中缺的工具记下来，一起把它做成下一次创作的帮手。</p></div>${external(community+'/issues/new?template=03-tool-need.yml','提出工具需求','site-row-link')}</div><div class="participation-row"><span class="step-number">04</span><div><h3>让 Agent 帮你投稿</h3><p>把 Skill 交给你的 Agent，按统一模板整理游戏、检查内容并提交 PR。新作品默认进入目录，首页精选由维护者挑选。</p></div>${external(community+'/blob/main/skills/openaigames-submit-demo/SKILL.md','读取 Agent Skill','site-row-link')}</div><div class="community-local"><div><span class="site-kicker">START SMALL, PLAY NOW</span><h2>还没有作品？先搓一张。</h2><p>先来试玩社区里的作品，再把你的感受和想做的玩法留在留言板。</p><small>做出能玩的版本，再带上试玩地址和制作过程回到社区。</small></div><a class="site-button primary" href="#/games">试玩社区作品 <span>→</span></a></div><div class="community-footnote">站内分享愿望和试玩感受，源码协作与投稿在 GitHub 继续。${external(community+'/blob/main/CONTRIBUTING.md','阅读参与指南')}</div></section>`);
  }
  async function route() {
    const epoch = ++routeId;
    request?.abort(); request = new AbortController();
    const [kind, id] = paths();
    pageKind = ['games','title','community','board'].includes(kind) ? kind : '';
    ...
    document.title = `${pageKind === 'games' ? '游戏目录' : pageKind === 'board' ? '许愿 / 留言' : pageKind === 'community' ? '如何共创' : '社区游戏展厅'} · OpenAIGames`;
